 ...
 * - if it does check: friendship:
 *  ** if 0 -> update, set to 1; f_status (0) !!
 *  ** if 1 -> check: f_status
...
// 1. create new row (romantic (0), friendship (1), r_status (0), f_status (0), created at (timestamp)
...
$stmt_upd1 = $conn->prepare("UPDATE relationship SET friendship=1 WHERE
    user_id1 = ? AND user_id2 = ?"); 
    // case 1: id1 is sess id, case 2: id2 is sess id

// 3. row exists, friendship is 1, status is 0; sess id is id2
$stmt_upd2 = $conn->prepare("UPDATE relationship SET f_status=1 WHERE
    user_id1 = ? AND user_id2 = ?");
    // id1 = target id, id2 = sess id


// In addition, we need the queries to see if the row exists
$stmt_exist = $conn->prepare("SELECT friendship, f_status from relationship WHERE
    user_id1 = ? AND user_id2 = ?");
...
if (isset($_POST["target_id"])) { // submit has been pressed, target_id has been set
    ...
    // next step: check friendship bit:
    // if friendship=0 change to 1
    ...
    // if f_status=1: do nothing
    // if f_status=0 and id1=1: do nothing
    // if f_status=0 and id1=0: update f_status to 1
    } elseif ($exist["f_status"] == 0 && $id1==0) { // update
            // id1 = target id, id2 = sess id
